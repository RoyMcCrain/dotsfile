...
- **Model IDs**: `pi/agent/model-roles.json` is the single source of truth. Skills and runners reference roles (`review.cursor`, `review.codex`, `review.claude`, `review.fugu`, `impl.cursor`, `research.xai`), never literal model IDs. Resolve Pi roles with `~/.pi/agent/resolve-model.sh ROLE`; resolve Cursor Agent roles with `--field cursor ROLE`; list with `--list`; after editing the catalog run `--apply` to sync `enabledModels` into `settings.json`.
