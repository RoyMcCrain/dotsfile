...
- **Model budget**: Fugu は週次 quota が厳しいため主モデルには自動選択しない。単体 `fugu-review` はユーザーが明示した場合だけ。`parallel-review` は level 3（deep）のみ fugu-ultra を reviewer に含む。quota/rate-limit 時は再試行しない。長いセッションは `/new` または `/compact` で区切る。
...
- **Review**: Plain 「レビューして」 uses `parallel-review`: one secret-filtered patch, isolated Pi headless reviewers in parallel at a chosen tier (`reviewLevels` 1=light, 2=standard/default, 3=deep). Tiers select model precision and fixed attempt budgets from `reviewTimeouts` (L1 300/300s, L2 600/600s, L3 600/900s initial/retry). Fugu is included only at level 3. Each non-Fugu reviewer is retried once after any failure (including timeout); the fugu reviewer is never retried to protect its quota. All reviewers configured for the tier run, including the reviewer whose provider matches the current session (`PI_PROVIDER`). Child reviewers must disable skills/context to prevent recursive review spawning. Use a single-reviewer skill only when named (`codex-review`, `claude-review`, `fugu-review`, `grok-review`).
