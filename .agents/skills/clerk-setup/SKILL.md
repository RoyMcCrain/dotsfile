...
| 2. Fetch quickstart | Use the host's web retrieval tool on the appropriate docs URL |
...
User Request: "Add Clerk" / "Add authentication"
    ...
    │   └─ NO → Fresh install
    │
    ├─ Identify framework → Fetch quickstart with the host's web retrieval tool → Follow instructions
...
Use the host's web retrieval tool to retrieve the official quickstart for the detected framework:

```
URL: https://clerk.com/docs/{framework}/getting-started/quickstart
