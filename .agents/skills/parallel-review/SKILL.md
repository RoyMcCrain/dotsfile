...
description: 並行コードレビューを実行。「レビューして」と言われたらまずこれを使う。Pi上では Cursor + Codex、または Cursor + Claude を使う。単体レビューは cursor-review/codex-review/claude-review/fugu-review を使う。
metadata:
  target_agent: Codex
---
...
- デフォルト: **Cursor + Codex**
- ユーザーが `claude` / `Claude` / `opus` を明示した場合: **Cursor + Claude**
