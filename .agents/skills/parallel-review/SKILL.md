...
description: 並行コードレビューを実行。「レビューして」と言われたらまずこれを使う。Pi上では Grok via Cursor + Codex、または Grok via Cursor + Claude を使う。単体レビューは cursor-review/codex-review/claude-review/fugu-review を使う。
metadata:
  target_agent: Codex
---
...
Pi 上で、2つの外部レビュアーを並行実行して結果を統合するスキル。
...
- Pi で明示実行する場合は `/skill:parallel-review [レビュー対象・観点]`

## Pi 上での組み合わせ

- デフォルト: **Grok via Cursor + Codex**
- ユーザーが `claude` / `Claude` / `opus` を明示した場合: **Grok via Cursor + Claude**
- Fugu Ultra の単体レビューが必要な場合は `/fugu-review` を使う。
