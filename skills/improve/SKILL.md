...
> **VCS note**: This skill's examples use `git`. If the audited repo has a `.jj` directory, use the `jj` equivalents instead (see the VCS instructions in the repository's `AGENTS.md`) — e.g. `git log --oneline -30` → `jj log -T ... -n 30`, `git diff --name-only $(git merge-base origin/<default> HEAD)..HEAD` → `jj diff --from 'trunk()' --to @ --name-only`.
