...
中学生でもわかるような1枚のHTMLアーティファクトに図解も入れつつ作成してください。

概要は専門用語に短い説明を添え、要点と図解を1枚の `report.html` にまとめる。
詳細説明・根拠・差分は「HTML1枚」の制約対象外とし、必要なら別の HTML に分けて概要からリンクする。
図解は根拠のある内容のみとし、既存の JSON contract・図解制約・merge 時の保持ルールを守る。
...
- 既定 reviewer は role `review.codex`、timeout 180秒。`review.claude` は
  fallback。Fugu（`review.fugu`）は明示依頼時のみ（`fugu-review` skill）。実モデル ID は
