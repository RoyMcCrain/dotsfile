...
Composer 2.5 Fast に実装まで投げるスキル。呼び出し元エージェント（Claude / Codex / Pi等）が
「正確な実装プロンプト」を作って渡し、実装後に呼び出し元エージェント自身が検証する。

## 基本方針

**実装は積極的に cursor-impl に委譲する。呼び出し元エージェントが直接書くより速いため、Composer 2.5 Fast をデフォルトにする。**
呼び出し元エージェントは「touchpoint の地図化・プロンプト作成・検証」に専念し、コード編集は Composer に任せる。
...
5. **呼び出し元エージェントによる検証**（必須）:
   ...
   - test（関連テスト）
   - 確定仕様の充足チェック
6. **問題があれば**: 軽微なら呼び出し元エージェントが直接修正、大きければ追加プロンプトで Composer に再投げ
...
- **共有グルーコード（union 型・登録 index・constants map・exhaustive switch 等）は呼び出し元エージェント自身が書く**。Composer には独立した新規ファイルや閉じた編集だけを任せる → 競合ゼロにできる
- 呼び出し元エージェントの touchpoint 地図を「重複しないファイル集合」N 個に分割し、各 Composer には**自分のバッチのファイルだけ**を渡す
- 同じファイルを複数 Composer が触らざるを得ないなら、それは分割し直すか直列化するサイン（並行しても結局マージで詰まる）

### 手順

1. touchpoint を disjoint なバッチに分割（共有ファイルは除外して呼び出し元エージェント担当に回す）
